...
| `rmapi`, `SYS1000/` paths | reMarkable / sandboxed embedded |
...
- For PowerShell orchestration: registry-driven dispatch, headless session layer, identity fields
- For Crippa/hardware: false-success → moved-material safety
- For static web: helper-first reuse, three-file architecture
...
| **PowerShell orchestration** | Registry-driven dispatch; canonical persistence helpers (no raw `Set-Content`); headless session layer (no `Write-Host` in `src/session/`); identity field rules |
