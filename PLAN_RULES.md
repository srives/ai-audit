...
This governance document defines how a **wish** becomes a **plan**. It is the policy the planning step consumes: the design process reads it as its governing rules and drafts, debates, and converges on the plan under them.

It is installed into a project's governance payload at bootstrap and resolved through the repo's normal governance order. Three audiences are bound by it:

- **AI planners** — the workers inside the design step that draft, debate, and converge on the plan.
- **AI executors** — the workers that later implement the plan's slices and are reviewed under `PR_TOUGH.md`.
...
**Method catalog.** The operational methods this policy invokes are installed prompt assets under `prompts/` (data resources, not specs):

| Method | Used at | Purpose |
|---|---|---|
| `prompts/TheSortingHat.md` | Stage 1 | Classifies a wish/repo against the installed stack catalog; produces ranked candidates with fitness scores. This is the machinery behind stack selection during design. |
...
| `prompts/MakeOutsideRepo.md` | project bootstrap | Orchestrates Sorting Hat → optional taxonomy creation → expert population → target governance install. |
...
- The wish MUST be a typed, governed wish artifact — a structured wish file, not a loose request string.
...
- When the wish is ambiguous about outcome, scope, or constraints, the planner MUST use clarification mode rather than guess. Clarification results land on the plan (assumptions, questions, user answers); a plan built on unrecorded guesses is defective.
...
1. **Check whether the wish already carries its territory.** A governed wish file may declare its stack. If it does, use it; do not re-classify what the human already decided.
2. **Derive the stack explicitly when absent.** Use a visible, recorded selection step, not hidden classification: when the wish carries no stack, derive one from the wish via the classification method and record the derivation on the plan.
3. **Gate on fitness — the Sorting Hat thresholds.** A derived stack carries a transient fitness score (0–100), produced by the classification method in `prompts/TheSortingHat.md`. The plan MUST record the selected stack's identity (code and name) and its fitness, and obey the canonical thresholds:
   - **85–100** — strong/direct fit; use the stack as the working default.
   - **50–84** — partial fit; the planner MUST NOT silently proceed. Present close candidates to the human, or stop and ask whether taxonomy refinement or a new taxonomy is needed.
...
4. **An explicit stack choice supplied to the design step always wins** over the wish's declared stack. The plan records which path selected the territory and why.
...
7. **Target-shape citation or rejection.** Any plan text that names a target output shape must cite the exact authority that makes it real: a spec section, protocol line range, generated protocol snapshot, prototype source location, or checked-in derivation artifact. This applies to fields, adapters, route keys, artifact maps, manifests, provenance, evidence, policy locations, approval gates, and any other target noun. If the target location cannot be cited exactly, the plan must mark it unknown-target, later-track, or contract-decision gap. Plausible target nouns are not evidence.
...
- exact citations for every target shape the plan relies on, or explicit unknown-target / later-track / contract-decision treatment;
...
Judgment evaluates the work against the governing policy and the done standard; the human owns the commit. An executor's summary saying "done" carries no evidentiary weight on its own — completion is an evaluated done-standard, never a printed claim of done.
