...
> **⚠ PLACEHOLDER — TAILOR AT INSTALL.** This is the generic model `CLAUDE.md` that ships with the governance payload. The trailing underscore marks it as a template: when the governance payload is installed on a client machine, this file is tailored to the target project (Section 0 and the platform addenda are replaced or trimmed to the client project's specifics) and renamed to `CLAUDE.md`. Until tailored, Section 0 is a `<tailor at install>` placeholder — do not treat it as instructions for the current repository. The universal sections (1–7) apply as-is to any project.

Guidance for Claude Code (claude.ai/code) working in this repository.

Synthesized from production CLAUDE.md and AGENTS.md files across thirty-one repos spanning PowerShell orchestration, ASP.NET + MongoDB services (Stratus), WPF desktop apps (Workstation, Print, Tooling), C++ experiments, embedded Linux integration, Revit / BIM plugins (WireWorks), Python reporting tools, Node/Electron session managers, retro-style custom-chrome games, and static sites.

The universal sections (0–7) apply to any project. Platform-specific addenda (A–L) at the bottom apply only when the work touches that platform. `<Tailor at install: name the target repo here and point to the platform addendum (A–L) that carries its immediate working rules.>`

---

## 0. Project Overview (THIS Repo — `<tailor at install>`)

`<Tailor at install: replace this section with the target project's overview — what the project is, the key files or projects that make it up, and any architectural commitments (e.g., zero-dependency, no build step) that every session must know.>`

### Running Locally

`<Tailor at install: the exact command(s) to build and run the project locally.>`

### Deployment

`<Tailor at install: how changes reach production — branch, pipeline, environment.>`

`<Tailor at install: point to the platform addendum (A–L) that carries this project's platform-specific mechanics.>`
...
1. **Architecture and Namespace Drift (Critical)** — Does new code violate the documented architecture for this repo? For example, in a three-file static site: does content logic leak into `app.js`? Does runtime logic appear inline in `index.html` outside of quiz data arrays?
...
Applies to: single-file WPF overlays, MVVM apps (Workstation, Print, Tooling), retro-game / custom-chrome overlays, and any WPF surface that mutates settings or talks to hardware.
...
### A.7. Game-loop discipline (retro overlays)
...
### A.8. Owner-drawn controls (retro game UIs)
...
## F. PowerShell Orchestration

Applies to: any PowerShell system with canonical session authority and registry-driven platform behavior.
...
- `sessionKey` is the immutable primary key; `nativeSessionId` is the alternate platform key.
...
Applies to: any single-purpose C++ experiment driven by CMake + MSVC.
...
## H. Static HTML/JS/CSS Sites

Applies to: any zero-dependency static site with three files (HTML + JS + CSS).
...
## I. Embedded Linux Integration

Applies to: any project where code interacts with a sandboxed root directory on a device and must not escape it.

### I.1. Transport abstraction as source of truth
All cloud/device operations through a single transport client module (e.g., `src/cloud/client.ts`). No direct transport-library calls outside that module.
...
Any file operation that could escape the documented sandbox root on the device is a critical defect. Path-traversal vulnerabilities (`../` in user-supplied paths), symlink resolution leaving the sandbox, and absolute paths where sandbox-relative is required are blocking.
...
Applies to: any WPF / WinForms surface that emulates older UI aesthetics over the operator's terminal or desktop.
...
### L.5. Era-faithful math (1958-1986 era simulators)
