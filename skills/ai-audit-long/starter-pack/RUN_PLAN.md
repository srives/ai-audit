...
Synthesized from production run plans across PowerShell orchestration engines, ASP.NET + MongoDB services (Stratus), WPF desktop apps (Workstation, Print, Tooling), C++ experiments, and static web projects. Platform-specific guidance lives in the dedicated sections at the bottom; everything above the dividing line applies to all work.
...
Applies to: WPF overlay tools, WPF MVVM apps (Workstation, Print, Tooling), any WPF surface that mutates settings or talks to hardware.
...
If the WPF app is a sidecar to a larger system (a monitor in front of an orchestration engine, Workstation in front of the cloud backend), every command it submits must use a command name the engine actually has. Do not invent commands. Grep the engine's command surface to confirm.
...
## F. PowerShell Orchestration / Session Authority

Applies to: any PowerShell system with canonical session authority, registry-driven platform behavior, and orchestration runtime.

### F.1. Headless session layer

- Files in the headless session layer must not contain `Write-Host`, `Read-Host`, `Read-DialogInput`, `Read-MenuKey`, `Confirm-Destructive`, `Confirm-YesNo`, or `$host.UI.RawUI.ReadKey`.
- All interactive output, prompts, menus, and colored rendering belongs in the UI layer.
- If a session function needs to communicate with the user, return data and let the UI layer render it.

### F.2. Identity field discipline

- The only identity keys are the canonical session key (primary) and `nativeSessionId` (alternate platform key).
...
- `wtProfileName` must not appear in canonical session records or as a parameter on `Update-SessionRecord`.
...
- Write to the canonical session record first, then refresh optional projections, then refresh presentation. Never write a projection first.
- Never treat optional projection/cache data as authoritative input to a mutation decision.
- A named launch creates canonical session identity (session key and canonical record) immediately, even before the platform exposes a native session ID. Do not fake a native session ID; leave `nativeSessionId` empty until uniqueness-proven backfill.

### F.5. Startup / refresh boundary

| Mode | Rules |
|---|---|
| **Startup** | Authority-read, offline, fast. Read non-deleted canonical session records. Build projections. NO live discovery, NO lifecycle writes, NO repair. Must complete from local authority reads only. |
| **Post-launch return** | Reread the just-written canonical session record. Do NOT trigger full live discovery. Do not invent a third in-memory authority. |
| **Refresh** | Run `Sync-SessionAuthority`. Re-read canonical session records. Build projections. Refresh never renders discovery output directly. |
...
If the plan touches the orchestration engine's state modules or hooks, workflow status/control surfaces, workflow execution state, run artifacts, signals, retries, or approvals:

1. **One supervisor only.** Exactly one runtime path is allowed to mutate canonical workflow state. Hooks, UI actions, approvals, retries, worker completion, and transport ingress must all submit to the same command/supervisor path. External tools (monitors, dashboards) must submit commands -- they must not hand-edit workflow snapshots.
...
6. **Freeze the execution definition.** A running workflow must never re-read a mutable external execution-definition file as authority.
...
9. **Operator sidecars are clients, not supervisors.** If a sidecar needs a new recovery action, add an engine command and have the sidecar call it. Sidecars must use the engine's documented command surface. The sidecar must not invent commands the engine does not have.
...
- Run the repo's fast changed-files lint (e.g., `lint.cmd --changed`) immediately after writing code. Fast (under 5s).
- Run the staged-files lint (e.g., `lint.cmd --staged`) before commit.
- Use the repo's validation planner to choose commands instead of guessing.
- Use fail-fast / delta modes of the repo's validation runner for repeated narrow checks.
- Run the repo's build script when source changed.
...
Applies to: any single-purpose C++ experiment driven by CMake and MSVC.

### G.1. Reusability layering

Separate reusable platform-concept code from application-specific code. Concept files are one concept per file. No concept file may know about the application's domain. They take generic inputs (paths, GUIDs, strings, byte buffers) and return generic outputs.
...
Applies to: static single-page sites with three files (HTML + JS + CSS).
...
- **Anti-pattern that shipped:** the workflow snapshot read helper persisted its rebuilt snapshot on read, so a monitor glance regressed state behind a committed command.
...
- **Anti-pattern that shipped (CRITICAL):** `complete-step`/`fail-step` checked the fence; `retry-step` checked nothing and forced `running` — a retry the instant after Stop resurrected a cancelled run.
...
- **Anti-patterns that shipped:** unreadable canonical session record → overwritten with a blank husk; one torn journal line → whole execution bricked; offline Claude folder → every record deleted.
